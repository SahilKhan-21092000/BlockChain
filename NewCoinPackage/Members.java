...
public class Members {
  ...
  public void initiateCoinsend(String destUID, NewCoin_Honest NewCobj) {
    ...
    for (int i = 0; i < NewCobj.memberlist.length; i++) {
      if (NewCobj.memberlist[i].UID.equals(destUID)) {
        obj.Destination = NewCobj.memberlist[i];
        break;
      }
    }
    ...
    NewCobj.pendingTransactions.AddTransactions(obj);

  }

  // Initiating coin send in Malicious BlockChain

  public void initiateCoinsend(String destUID, NewCoin_Malicious NewCobj) {
    Pair<String, TransactionBlock> pr = mycoins.remove(0);
    Transaction obj = new Transaction();
    obj.coinID = pr.get_first();
    obj.coinsrc_block = pr.get_second();
  ...
  public Pair<List<Pair<String, String>>, List<Pair<String, String>>> finalizeCoinsend(Transaction tobj,
      NewCoin_Honest NewCObj) throws MissingTransactionException {
    TransactionBlock temp = NewCObj.bChain.lastBlock;
    ...
    TransactionBlock key = NewCObj.bChain.lastBlock;
  ...
  public void MineCoin(NewCoin_Honest NewCObj) {
    ...
    while (i < (NewCObj.bChain.tr_count - 1)) {
      int flag = 0;
      try {
        Transaction temp = NewCObj.pendingTransactions.RemoveTransaction();

        // Validity of transaction check

        if (checkTrans(temp, NewCObj.bChain.lastBlock) == false) {
          continue;
        }
    ...
    int x = Integer.parseInt(NewCObj.latestCoinID) + 1;

    NewCObj.latestCoinID = Integer.toString(x);
    minerRewardTransaction.coinID = NewCObj.latestCoinID;
    ...
    arr[NewCObj.bChain.tr_count - 1] = minerRewardTransaction;

    TransactionBlock tB = new TransactionBlock(arr);

    NewCObj.bChain.InsertBlock_Honest(tB);

    Pair<String, TransactionBlock> pr = new Pair<String, TransactionBlock>(NewCObj.latestCoinID, tB);
  ...
  public void MineCoin(NewCoin_Malicious NewCObj) {

    Transaction[] arr = new Transaction[NewCObj.bChain.tr_count];

    int j = 0;
    for (int i = 0; i < (NewCObj.bChain.tr_count - 1); i++) {
      int flag = 0;
      Transaction temp;
      try {
        temp = NewCObj.pendingTransactions.RemoveTransaction();

        // Validity check for Malicious transaction

        if (checkTrans(temp, NewCObj.bChain.FindLongestValidChain()) == false) {
          i--;
          continue;
        }
